    ...
    <AppShell>
      <header className="flex flex-wrap items-end justify-between gap-3 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => downloadFile(`kahani-packs-${Date.now()}.csv`, packsToCsv(filtered), "text/csv")}
          >
      ...
      <div className="card-soft p-4 mb-6 grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
        <div className="relative sm:col-span-2">
          ...
          <input className="input-field pl-9" placeholder="Search by title…" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
        <FilterSelect value={platform} onChange={setPlatform} options={PLATFORMS} placeholder="All platforms" />
        <FilterSelect value={language} onChange={setLanguage} options={LANGS} placeholder="All languages" />
      ...
      {filtered.length === 0 ? (
        <div className="card-soft p-10 text-center text-muted-foreground">
          No packs match these filters. <Link to="/create" className="text-primary underline">Create one</Link>.
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p) => (
            <Link key={p.id} to="/packs/$id" params={{ id: p.id }} className="card-soft p-5 block hover:-translate-y-0.5 hover:shadow-[var(--shadow-lift)] transition-transform">
              ...
              {p.scheduledDate && <p className="text-xs mt-3 text-muted-foreground">Scheduled {new Date(p.scheduledDate).toLocaleDateString()}</p>}
            </Link>
          ))}
        </div>
      )}
